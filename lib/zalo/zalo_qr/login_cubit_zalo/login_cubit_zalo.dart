...
part 'login_state_zalo.dart';

class LoginCubitZalo extends Cubit<LoginStateZalo> {
  // LoginCubitZalo() : super(LoginStateInit());

  final AuthRepo _authRepo = AuthRepo();

  static LoginCubitZalo? _instance;

  factory LoginCubitZalo() => _instance ??= LoginCubitZalo._();

  LoginCubitZalo._() : super(LoginStateZaloInit());
  ...
  //       chatClient.emit(
  //           ChatSocketEvent.login, [userInfo.id, StringConst.fromChat365]);
  //       if (data.warning == 1 &&
  ...
  Future<void> getDataQR() async {
    await chatClientZalo.emit('join', [
      {'id_chat': AuthRepo().userInfo!.id}
    ]);
    await chatClientZalo.emit('login', [
      {'id_chat': AuthRepo().userInfo!.id}
    ]);
    // chatClientZalo.emit('update_list_chat', {
    //   'id_chat': AuthRepo().userInfo!.id,
    //   'id_zalo': userInfoZalo.idZalo,
    //   'check_first': true
    // });
  }

  Future<void> getDataConversation() async {
    await chatClientZalo.emit('update_list_chat', {
      'id_chat': AuthRepo().userInfo!.id,
      'id_zalo': userInfoZalo.idZalo,
      'check_first': true
    });
    // chatClientZalo.emit('thai_dui', {'id_chat': AuthRepo().userInfo!.id});
  }

  void listenForQRLoginSocket() async {
    await getDataQR();
  }
