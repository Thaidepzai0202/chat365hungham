...
import 'package:app_chat365_pc/common/Widgets/list_contact_view.dart';
...
class _SendButtonState extends State<SendButton>
  ...
  @override
  void initState() {
    print('\x1B[35m senderID: ${widget.senderInfo}');
    ...
    _subscription = _chatBloc.stream.listen((current) {
      if (current is ChatStateSendMessageSuccess &&
        ...
        BotToast.showText(text: current.error.error);
      }
    });
  ...
  ///1
  _onPressed() async {
    logger.log('ccccccccc');

    if (_stateNotifier.value != DialogState.init) {
      // TL 18/1/2024: Lấy CTC từ ChatRepo
    ...
    if (widget.apiMessageModel.contact != null) {
      fullInfoContact =
          await UserInfoRepo().getUserInfo(widget.apiMessageModel.contact!.id);
    }
    ...
    if (conversationId == -1) {
      try {
        ...
        BotToast.showText(text: 'Gửi tin nhắn thất bại');
    ...
    var listMessage = SystemUtils.getListApiMessageModels(
      ...
      messageId: newMessageId,
      replyModel: newMessageModel.replyMessage,
      text: widget.newMessage.value,
      infoLink: widget.infoLink,
      infoLinkMessageType: widget.apiMessageModel.type,
    );
    // Nếu là type là contact thì gửi list còn không thì gửi newMessageModel
    var message1 = listMessage.first;
    if (message1.type != MessageType.contact &&
        message1.type != MessageType.link) {
      _chatBloc.sendMessage(newMessageModel, memberIds: []);
    } else {
