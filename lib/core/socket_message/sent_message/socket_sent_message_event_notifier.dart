...
typedef SocketSentMessageCallback = void Function(
    SocketSentMessageModel message);

class SocketSentMessageEventNotifier {
  int _count = 0;
  List<SocketSentMessageCallback?> _listeners =
      List<SocketSentMessageCallback?>.filled(0, null);
  ...
  void addListener(SocketSentMessageCallback listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<SocketSentMessageCallback?>.filled(1, null);
      } else {
        final List<SocketSentMessageCallback?> newListeners =
            List<SocketSentMessageCallback?>.filled(
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<SocketSentMessageCallback?> newListeners =
          List<SocketSentMessageCallback?>.filled(_count, null);
  ...
  ///    changes.
  void removeListener(SocketSentMessageCallback listener) {
    assert(_debugAssertNotDisposed());
    for (int i = 0; i < _count; i++) {
      final SocketSentMessageCallback? _listener = _listeners[i];
  ...
  void notifyListeners(SocketSentMessageModel message) {
    assert(_debugAssertNotDisposed());
    if (_count == 0) return;
      ...
      try {
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () sync* {
            yield DiagnosticsProperty<SocketSentMessageEventNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<SocketSentMessageCallback?> newListeners =
            List<SocketSentMessageCallback?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final SocketSentMessageCallback? listener = _listeners[i];
          if (listener != null) {
